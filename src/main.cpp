/*Pues este es un experimento, la neta creo que   sería interezantehacer un
 * videojuego con esto y ver hasta donde puedo llegar, en plan de que ¿podría yo
 * hacer un motor de videojuegos aquí? hasta donde podré llegar? por ahora voy a
 * intentar hacer todo aquí a ver hasta donde puedo llegar empezemos   por algo
 * simple*/

/*Terminar movimientoJugador(){} linea: 196
 * Creo que al final de reaccionar input pestana 2 hay que agregar una función
 * que se asegure la camara no intente salirse del mapa*/
/*
#ifdef __ANDROID__
    // Si estamos en Termux/Android, apagamos los servidores de audio de
escritorio #define MA_NO_PULSEAUDIO #define MA_NO_ALSA #define MA_NO_JACK
#endif*/
...
// Una entidad será cualquier cosa que tenga un símbolo en mapa y una posición X
// Y
...
class FlujoJuego {
  ...
  void archivoTxtA_Vector(std::string nombreArchivo,
                          vector<vector<char>> &vectorVacio) {

    std::ifstream archivo(nombreArchivo); // abrir archivo
    char tile;                            // para leer el archivo es x linea
    vectorVacio.push_back(
        vector<char>()); // Coss a mejorar: hacer un if por si algo raro pasa y
                         // tengo un tileMap con 1 fila o más
    int filas = 0;
  ...
  void dibujarMapPantalla(const Camara &posicionCamara,
    ...
    std::system("clear");
#endif
    for (int numeroFila = posicionCamara.inicioY;
         numeroFila <= posicionCamara.finalY; numeroFila++) {

      for (int numeroColumna = posicionCamara.inicioX;
           numeroColumna <= posicionCamara.finalX; numeroColumna++) {
        // capás y lo wue sigue como otra función
        if (numeroFila == jugador1.posicionY &&
            numeroColumna == jugador1.posicionX && indicadorPestana == 2) {
  ...
  int indicadorPestana; // indica en qué pestaña está el usuario en el juego, el
                        // menu, la pantalla de título, el mini mapa, etc.
  ...
  char checkInput() {
    ...
    do {
      ...
      if (input == 'e' || input == 's' || input == 'd' || input == 'f' ||
          input == 'u' || input == 'j' || input == 'i' || input == 'k' ||
          input == 'q' || input == 'p') {
  ...
  } esto ya no me sirve porque así no dibujo arriba del mapa y cuando me mueva
  tendría que guardar el tile de abajo de la última posición de todas las
  entidades y remplazarlo después de que cada una se mueva*/
  ...
  void movimientoJugador(int input) {
    ...
    if (input == 'f' &&
        jugador1.posicionX < tileMap[jugador1.posicionY].size() - 1) {
      jugador1.posicionX += 1;
    }
  }
  ...
  void reaccionarInput(char input) {
    ...
    if (indicadorPestana == 1) {
      ...
      cout << "algo salió mal, indicadorPestana no tiene un parametro valido"
           << endl;
    }
  }
...
public:
  FlujoJuego()
      : juegoCorriendo(false), tileMap(vector<vector<char>>()),
        mapaMenu(vector<vector<char>>()), indicadorPestana(0) {}

  void inicializar() {

    // Pantalla carga para los dos. Cargar todo, Inicializar todo + primera
    // pantalla
    cargarDatos();
  ...
  void ejecutar() {
    cout << "ejecutando programa"
         << endl; // hacer una función para cualquier debug de cualquier parte
                  // del codigo antes y después de la inicialización de la
                  // primera pantalla.
